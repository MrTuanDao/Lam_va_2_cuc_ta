...
void autoKillProcess() {
    ...
    for (int k = 0; k != process_name_unique.size(); k++)
        for (int j = 0; j != process_name_unique.size(); j++) {
            if (strcmp(process_name_unique[k].c_str(), process_name_unique_unsorted[j].c_str()) == 0)
                process_num_occur_sorted.push_back(process_num_occur[j]);
        }
  
    SetTeColor(2);
    cout << "---------------Child processes that are running------------------" << endl;
    cout << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$" << endl;
    cout << "PROCESS NAME || Number of processes of the same name                                " << endl;
    for (int k = 0; k <= (process_name_unique.size() - 1); k++) {
        if (processList.find(process_name_unique[k]) == processList.end()) continue;
        cout << process_name_unique[k];
        cout << " || " << process_num_occur_sorted[k] << endl;
    ...
    cout << endl << endl;
    cout << "For example, enter 'clock.exe' to terminate all processes of that handle name." << endl;
    cout << "(Exit the program if you do not wish to progress)" << endl;
    cout << "" << endl;
    cout << "Process (name) to terminate:";
    Sleep(500);
